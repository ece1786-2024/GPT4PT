...
class TfClassifier:
    ...
    def _init_gpt4(self):
        prompt_path = './tf_agent/prompt.txt'
    ...
    def classify(self, text: str):
        ...
        >> [0.3, 0.7]
        '''
        encoded_input = self.tokenizer(text, truncation=True, padding=True, max_length=1000, return_tensors="pt")
        outputs = self.gpt2(**encoded_input)
        logits = outputs.logits
        return self._to_probs(logits)
    
    def _generate_question(self, probs: list, conversation):
        
        conversation.pop()
        conversation.append({"role": "system", "content": f'Thinking: {probs[0]*100}%, Feeling: {probs[1]*100}%'})
    ...
    def start(self):
        ...
        conversation.append({"role": "system", "content": f'Thinking: 0%, Feeling: 0%'})

        # First question start with 0% and 0%
        question = self._generate_question([0,0], conversation)
        ...
        question = self._generate_question(probs, conversation)
        answer = input(f'Question: {question}\n')
        responses.append(question)       
        responses.append(answer)        
        probs = self.classify('\n'.join(responses))
        ...
        return 'F' if probs[0] > probs[1] else 'T'
