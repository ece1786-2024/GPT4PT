...
class JpClassifier:
    ...
    def _generate_question(self, prompt, probs: list):
        #print(f'Judging: {probs[0]*100}%, Perceiving: {probs[1]*100}%')
        return self.gpt4.chat.completions.create(
            model="gpt-4o",
            messages=[{"role": "system", "content": prompt}] + 
            [{"role": "user", "content": f'Judging: {probs[0]*100}%, Perceiving: {probs[1]*100}%'}],
            temperature=0.7,
            #temperature=0.5,
            top_p=1
    ...
    def start(self):
        ...
        return 'P' if probs[1] > probs[0] else 'J'